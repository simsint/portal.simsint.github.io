...
// }
let IMAGE_PATH = "";
if (window.location.hostname == "ahmedalnajjar.me" || window.location.port == "") {
  IMAGE_PATH = "./assets/img/";
}

const ENDPOINT = "https://api.ahmedalnajjar.me/dev";
...
var patient = {
  showUserProfile: function (pid) {
    asyncResponse("GET", "/admin/users", { filter_name: "sub", filter_value: pid }, (res) => {
      // @ts-ignore
      $.showModal({
        title: "USER PROFILE",
  ...
  newVisitDetails: function () {
    ...
    // @ts-ignore
    $(".sections.view-visit-details").find('input[name="file_no"]').addClass("input-no-edit").val(new Date().getTime());
    // @ts-ignore
  ...
  viewVisitDetails: function (created) {
    asyncResponse(
      ...
      (res) => {
        ...
        // @ts-ignore
        $(".sections.view-visit-details").find("input,select,textarea,.custom-control-label").addClass("input-no-edit");
    ...
    asyncResponse(
      ...
      (res) => {
        ...
        res.Items.forEach((item) => {
          // @ts-ignore
          $(`<li class="previous-records action" data-action="view-user-report" data-created="${item.created}">
                <span>${
                  // @ts-ignore
                  moment(item.created).format("LLL")
                }</span>
      ...
      (res) => {
        ...
        res.Items.forEach((item) => {
          ...
          $(`<li class="previous-records action" data-action="view-form" data-form-type='${
            ...
            <span class="d-block mb-1 text-uppercase font-weight-bold">${item.form_type.replaceAll("_", " ")}</span>
            <span>${
              // @ts-ignore
              moment(item.created).format("LLL")
            }</span>
    ...
    asyncResponse(
      ...
      (res) => {
        ...
        res.body.Items.forEach((item) => {
          // @ts-ignore
          $(`<li class="previous-records action" data-action="view-invoice" data-created="${item.created}">
                <span>${
                  // @ts-ignore
                  moment(item.created).format("LLL")
                }</span>
    ...
    asyncResponse(
      ...
      (res) => {
        if (res.statusCode == 200) {
          ...
          // @ts-ignore
          $(".sections.view-invoice").find("input,select,textarea,.custom-control-label").addClass("input-no-edit");
          // @ts-ignore
  ...
  newInvoice: function () {
    ...
    // $("#receiver-signature").remove();
    // @ts-ignore
    $(".sections.view-invoice").find('input[name="invoice_no"]').addClass("input-no-edit").val(new Date().getTime());
    // @ts-ignore
    $("#invoice").find("input[name=name]")[0].value = sessionStorage.getItem("pt-fullname").replace(/\s\s+/g, " ");
    // @ts-ignore
    $("#invoice").find(
      "input[name=date]"
    )[0].value = `${new Date().getFullYear()}-${new Date().getMonth()}-${new Date().getDate()}`;
  ...
  submitForms: function (obj) {
    ...
    asyncResponse("POST", `/users/${sessionStorage.getItem("pid")}/forms`, formData, (res) => {
      console.log(res);
      successToast("Successful");
      if (formData.form_type == "treatment_plan") {
        patient.showTreatmentPlanSection(0);
  ...
  latestReport(pid) {
    ...
    asyncResponse("GET", `/users/${pid}/forms`, { form_type: "adult_form", limit: 1 }, (res) => {
      if (res.statusCode != 200) {
        console.log(res);
        return;
      }
      let gender = res?.Items[0]?.form_data?.gender;
      if (res.Items.length) {
        // delete res.Items[0].form_data.name;
        // @ts-ignore
        js2form("report", res.Items[0].form_data);
        if (res.Items[0].form_data.hasOwnProperty("custom:emirates_id_front")) {
          // @ts-ignore
          $("#pt-report-eidf").attr("src", res.Items[0].form_data["custom:emirates_id_front"]);
        }
      }
      asyncResponse(
        "GET",
        ...
        (res) => {
          if (res.statusCode == 200 && res.Count > 0) {
            // drawChart(res.Items[0].survey);
            // console.log("SHAFAFIYA INCIATOR", res.Items[0].survey);
            drawShafafiyaChart(res.Items[0].survey.sections[0]);
            draw15ScreenQuestions(res.Items[0].survey.sections[1], gender);
          }
        }
      );
      asyncResponse(
        ...
        (res) => {
          if (res.statusCode == 200 && res.Count > 0) {
            ...
            $("#report input[name=survey_date]").val(
              // @ts-ignore
              moment(res.Items[0].survey.created).format("MMMM Do YYYY, h:mm:ss a")
            );
            drawChart(res.Items[0].survey);
          }
        }
...
function drawShafafiyaChart(data) {
  let yes = 0,
    no = 0,
    nearlyYes = 0,
    nearlyNo = 0;
  data.questions.forEach((question) => {
    if (parseInt(question.value) == 1) {
      yes++;
    } else if (parseInt(question.value) == 2) {
      nearlyYes++;
    } else if (parseInt(question.value) == 3) {
      nearlyNo++;
    } else if (parseInt(question.value) == 4) {
      no++;
    }
  });
  //@ts-ignore
  let report_2_chart = document.querySelector("#report-chart-2 canvas").chart;
  let dataset = [
    whatPercentOf(yes, data.totalQuestions),
    whatPercentOf(nearlyYes, data.totalQuestions),
    whatPercentOf(nearlyNo, data.totalQuestions),
    whatPercentOf(no, data.totalQuestions),
  ];
  report_2_chart.data.datasets[0].data = dataset;
  report_2_chart.update();
  //@ts-ignore
  document.querySelector("#report-chart-2 canvas").chart.config;
}

function draw15ScreenQuestions(data, gender = "male") {
  let total = 0;
  let gaugeImage = "",
    guageTotalImage = "";
  data.questions.forEach((question) => {
    ...
    let questionPercent = whatPercentOf(parseInt(question.value), questionTotal);

    //@ts-ignore
    $("#sq-list li")
      .eq(question.question - 1)
      .find("div")
      .find("span")
      .html(`${questionPercent}%`);
    //@ts-ignore
    $("#report-specific-conditioning tbody tr")
      .eq(question.question - 1)
      .find("td")
      .eq(1)
      .find("span")
    ...
    if (parseInt(questionPercent) < 20) {
      gaugeImage = `${IMAGE_PATH}gauge-1.png`;
    } else if (parseInt(questionPercent) < 40) {
      gaugeImage = `${IMAGE_PATH}gauge-2.png`;
    } else if (parseInt(questionPercent) < 80) {
      gaugeImage = `${IMAGE_PATH}gauge-3.png`;
    } else {
      gaugeImage = `${IMAGE_PATH}gauge-4.png`;
    }
  ...
  let totalPercent = whatPercentOf(total, 120);
  console.log(" total percentage - ", totalPercent);

  if (parseInt(totalPercent) < 23) {
    guageTotalImage = `${IMAGE_PATH}gauge-c-1.png`;
  } else if (parseInt(totalPercent) < 45) {
    guageTotalImage = `${IMAGE_PATH}gauge-c-2.png`;
  } else if (parseInt(totalPercent) < 68) {
    guageTotalImage = `${IMAGE_PATH}gauge-c-3.png`;
  } else {
    guageTotalImage = `${IMAGE_PATH}gauge-c-4.png`;
  }

  //@ts-ignore
  $("#report-chart-total-percent")
    .find("img")
    .attr("src", IMAGE_PATH + guageTotalImage);
  //@ts-ignore
  $("#report-chart-total-percent").find("span").html(`${totalPercent}%`);

  let part = "";
  if (parseInt(totalPercent) < 41.666) {
    part = "part_1";
  } else if (parseInt(totalPercent) < 49.999) {
    part = "part_2";
  } else if (parseInt(totalPercent) < 66.666) {
    part = "part_3";
  } else if (parseInt(totalPercent) < 74.999) {
    part = "part_4";
  } else if (parseInt(totalPercent) < 83.33) {
    part = "part_5";
  } else {
    part = "part_6";
  }
  ...
  //@ts-ignore
  $("#report-general-comment tbody").html(`
    <tr>
      <td class="text-justify">${checklist_2_indications.general_comment[part].case_desc[gender]}</td>
    </tr>
    `);
  //@ts-ignore
  $("#report-action-reason tbody").html(`
    <tr>
    <td class="text-justify">${checklist_2_indications.screeningQuestions[part].action}</td>
    <td class="text-center text-denger" style="font-size: 1.5em;font-weight: bold;">${checklist_2_indications.screeningQuestions[part].reason}%</td>
  </tr>
  `);
}

function whatPercentOf(num, total) {
...
function initPreview(form_type, form_data, enabled) {
  ...
  if (enabled) {
    ...
    // @ts-ignore
    $(`.forms.${form_type}`).find("input,select,textarea,.custom-control-label").addClass("input-no-edit");
    // @ts-ignore
    $(`.forms.${form_type}`).find("input,select,textarea,.custom-control-label").addClass("input-no-edit");
    // @ts-ignore
    ...
    // @ts-ignore
    $(`.forms`).find("input,select,textarea,.custom-control-label").removeClass("input-no-edit");
    // @ts-ignore
...
$("#patient-select").select2({
  ...
  ajax: {
    ...
    data: function (params) {
      ...
      if (params._type === "query:append" && sessionStorage.getItem("patient_select_pagination_token") != "") {
        query.PaginationToken = sessionStorage.getItem("patient_select_pagination_token");
    ...
    processResults: function (res) {
      if (res.hasOwnProperty("PaginationToken")) {
        sessionStorage.setItem("patient_select_pagination_token", res.PaginationToken);
      } else {
        sessionStorage.setItem("patient_select_pagination_token", "");
      }
      var items = [];
...
function renderUserHtml(data) {
  ...
  <img class="w-100 rounded-circle" src=${
    data.picture != null ? data.picture : `${IMAGE_PATH}user.svg`
  } style="max-width: 100px; height:100px" />
  </div>
  <div class="col">
      <div class="row">
          <div class="col">
              <h5 class="text-capitalize mb-0 font-weight-bold">${renderUserFullName(data)}</h5>
              <p class="d-sm-block">${data.email}</p>
              <p class="mb-0"><strong>DOB</strong>:  ${
                // @ts-ignore
                data.birthdate != null ? moment(data.birthdate).format("YYYY-MM-DD") : "-"
              }</p>
              <p class="mb-0"><strong>Age</strong>: ${data.birthdate != null ? findAge(data.birthdate) : "-"} Years</p>
          </div>
          <div class="col">
              <p class="mb-0"><strong>Nationality</strong>: ${data.locale != null ? data.locale : "-"}</p>
              <p class="mb-0"><strong>Phone</strong>: ${data.phone_number != null ? data.phone_number : "-"}</p>
              <p class="mb-0"><strong>Address</strong>: ${data.address != null ? data.address : "-"}</p>
              ...
              <div class="dropdown d-inline-block m-2">
                ...
                <div class="dropdown-menu text-capitalize">
                  <a class="dropdown-item" href="#" data-action="pt-report" data-pid=${data.sub}>Report</a>
                  <a class="dropdown-item disabled" href="#" data-action="">New Appointment</a>
                  <div class="dropdown-divider"></div>
                  <a class="dropdown-item" href="#" data-action="update-profile" data-pid=${data.sub} data-pt-email=${
    data.email
  } >Edit Profile</a>
  ...
  <div class="col-12 collapse" id="collapse-${data.sub}">
      <div class="row mt-2">
        <div class="col-sm-12 col-md-6 text-center"><img class="img-fluid eids" src="${
          data["custom:emirates_id_front"] || `${IMAGE_PATH}eidf.jpg`
        }" /></div>
        <div class="col-sm-12 col-md-6 text-center"><img class="img-fluid eids" src="${
          data["custom:emirates_id_back"] || `${IMAGE_PATH}eidb.jpg`
        }" /></div>
      </div>
  </div>
</div>`;
...
function serviceDetails(service) {
  var text = `${service.hasOwnProperty("service_type") ? `${service.service_type} | ` : ""}
  ${service.hasOwnProperty("service_amount") ? service.service_amount : ""}
  ${service.hasOwnProperty("service_currency") ? `${service.service_currency} | ` : ""}
  ${service.hasOwnProperty("service_duration") ? `${service.service_duration} min | ` : ""}
  `;
...
function renderAppointmentHtml(event) {
    ...
    <div class="col">
        <div class="row">
            <div class="col">
                ...
                <p class="mb-2">${eventTimeToString(event.start.dateTime, event.end.dateTime)}</p>
            </div>
            <div class="col-auto">
                <p class="mb-1"><strong>File No. </strong> ${event?.extendedProperties?.private?.file_no}</p>
            </div>
        </div>
        ...
        <p class="mb-1"><strong>Service Details: </strong> ${serviceDetails(event?.extendedProperties?.private)} 
        </p>`;
  if (event.organizer.email == event?.extendedProperties?.private?.doctor_email) {
    htmlContent += `<p class="mb-0"><strong>Doctor: </strong>${
      event.organizer.displayName || event.organizer.email
    }</p>`;
  } else {
    htmlContent += `<p class="mb-0"><strong>Doctor: </strong>${event.organizer.email}</p>
                    <p class="mb-1">Created By ${event.organizer.displayName || event.organizer.email}</p>`;
  }
                    ...
                    <div class="col">
                        <p class="mb-1 d-inline-block p-2 text-white rounded ${
                          event?.extendedProperties?.private?.event_status.toLowerCase() == "pending"
                            ? "bg-warning"
                            : event?.extendedProperties?.private?.event_status.toLowerCase() == "confirmed"
                            ? "bg-success"
                            : "bg-danger"
                        }"><strong>Status: </strong>${event?.extendedProperties?.private?.event_status}</p>`;
  ...
  if (event?.extendedProperties?.private?.event_status.toLowerCase() == "pending") {
...
function renderUserFullName(data) {
  let fullName = `${data.hasOwnProperty("given_name") ? `${data.given_name} ` : ""}
                          ...
                          <div class="row mb-5">
                              <div class="col">
                                  <p>Name: ${getUserFullName({
                                    email: event?.extendedProperties?.private?.patient_email,
                                  })}
                                  )}</p>
                                  <p>Email : ${event?.extendedProperties?.private?.patient_email}</p>
                                  <p>Appointment : ${
                                    // @ts-ignore
                                    moment(event?.start?.dateTime).format("LLL")
                                  }</p>
                                  <p>Chief Complaint :${event?.extendedProperties?.private?.chief_complaint}</p>
                              </div>
                              <div class="col-auto">
                                  <p>File No. ${event?.extendedProperties?.private?.file_no}</p>
                          ...
                          <div class="table-responsive">
                              <table class="table">
                                  ...
                                  <tbody>
                                      <tr>
                                          <td>Video Call</td>
                                          <td>${event?.extendedProperties?.private?.service_amount} ${
...
function renderPtDetails(data) {
  ...
  let htmlContent = `<div class="row">
    <div class="col-md-12 col-lg-5">
        <p class="mb-1"><strong>Patient Name: </strong>${renderUserFullName(data)}</p>
        <p class="mb-1"><strong>Patient Email: </strong> ${data?.email}</p>
        <p class="mb-1"><strong>Sex: </strong> ${data.hasOwnProperty("gender") ? data.gender : "-"}</p>
        <p class="mb-1"><strong>Emirates ID: </strong> ${
          data.hasOwnProperty("preferred_username") ? data.preferred_username : "-"
        }</p>
        <p class="mb-1"><strong>Age: </strong> ${findAge(data?.birthdate)}</p>
        <p class="mb-1"><strong>DOB: </strong> ${data.hasOwnProperty("birthdate") ? data.birthdate : "-"}</p>
    </div>
    <div class="col-12 col-md-8 col-lg-5">
        <p class="mb-1"><strong>Nationality: </strong>${data.hasOwnProperty("locale") ? data.locale : "-"}</p>
        <p class="mb-1"><strong>Patient Type</strong> -</p>
        <p class="mb-1"><strong>Mobile No: </strong>${data.hasOwnProperty("phone_number") ? data.phone_number : "-"}</p>
    ...
    <div class="col-sm-12 col-md-auto order-sm-1">
        <div class="w-100 text-center p-2"><img class="w-100 rounded-circle" style="max-width: 100px; height:100px" src=${
          data.hasOwnProperty("picture") ? data.picture : "../assets/img/user.svg"
        } /></div>
    </div>
</div>`;
  return htmlContent;
...
function drawChart(survey) {
  ...
  if (survey != null) {
    ...
    $.each(survey.sections, function (index, section) {
      let block = section.totalQuestions / (section.totalQuestions * SCALE_RANGE);
      let sum = 0;
      // @ts-ignore
...
function updateChart(labels, data) {
  ...
  user_report_chart.data.datasets[0].backgroundColor = backgroundColors;
  user_report_chart.data.labels = labels;
  user_report_chart.data.datasets[0].data = data;
  user_report_chart.update();
  // @ts-ignore
  chartConfig = document.querySelector("#user-report-chart canvas").chart.config;
}
...
$(".invoiceInputChange").on("change", function (e) {
  ...
  $.each(formData.service_type, function (index, item) {
    ...
    console.log(`formData.service[${item}].amount- ${formData.service[item].amount}`);
