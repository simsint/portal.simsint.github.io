...
$(document).on("click", ".action, a", function (e) {
  ...
  if (action === "pt-report-2") {
    patient.latestReport2($(this).data("pid"));
  }
...
var patient = {
  ...
  latestReport2(pid) {
    // @ts-ignore
    $("#pt-report-modal-2").modal("show");
    // @ts-ignore
    $("#pt-report-modal-2 form").trigger("reset");
    asyncResponse(
      "GET",
      `/users/${pid}/forms`,
      { form_type: "adult_form", limit: 1 },
      (res) => {
        ...
        if (res.Items.length) {
          // delete res.Items[0].form_data.name;
          // @ts-ignore
          js2form("report-2", res.Items[0].form_data);
          if (
            res.Items[0].form_data.hasOwnProperty("custom:emirates_id_front")
          ) {
            // @ts-ignore
            $("#pt-report-eidf-2").attr(
              "src",
              res.Items[0].form_data["custom:emirates_id_front"]
            );
          }
    ...
    asyncResponse(
      ...
      (res) => {
        if (res.statusCode == 200 && res.Count > 0) {
          ...
          draw15ScreenQuestions(res.Items[0].survey.sections[1]);
        }
      }
    );
  },
...
function drawShafafiyaChart(data) {
  ...
  //@ts-ignore
  let user_report_2_chart = document.querySelector(
    "#user-report-chart-2 canvas"
  ).chart;
  ...
  user_report_2_chart.data.datasets[0].data = dataset;
  user_report_2_chart.update();
  //@ts-ignore
  document.querySelector("#user-report-chart-2 canvas").chart.config;
}

function draw15ScreenQuestions(data) {
  let generalComment_case_desc = "",
    generalComment_ref = "",
    actionReson_action = "",
    actionReson_reason = "";
  let total = 0;
  ...
  data.questions.forEach((question) => {
    ...
    //@ts-ignore
    $("#report-2-specific-conditioning tbody tr")
    ...
    if (parseInt(questionPercent) < 20) {
      gaugeImage = "./assets/img/gauge-1.png";
    } else if (parseInt(questionPercent) < 40) {
      gaugeImage = "./assets/img/gauge-2.png";
    } else if (parseInt(questionPercent) < 80) {
      gaugeImage = "./assets/img/gauge-3.png";
    } else {
      gaugeImage = "./assets/img/gauge-4.png";
    }
    //@ts-ignore
    $("#report-2-specific-conditioning tbody tr")
      .eq(question.question - 1)
  ...
  //@ts-ignore
  $("#report-2-general-comment tbody").html(`
    <tr>
      <td class="text-justify">${checklist_2_indications.general_comment[part].case_desc}</td>
      <td class="text-center">
        <p class="text-danger" style="font-size: 1.5em;font-weight: bold;">${totalPercent}%</p>
        <p>(Ref:${checklist_2_indications.general_comment[part].ref})</p>
      </td>
    </tr>
    `);
  //@ts-ignore
  $("#report-2-action-reason tbody").html(`
  <tr>
          ...
          <div class="col">
              ...
              <div class="dropdown d-inline-block m-2">
                ...
                <div class="dropdown-menu text-capitalize">
                  ...
                  <a class="dropdown-item" href="#" data-action="pt-report-2" data-pid=${
                    data.sub
                  }>Shafafiya Report</a>
